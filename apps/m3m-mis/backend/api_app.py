...
app = FastAPI(title="M3M Customer Complaint MIS")
...
def export_excel_route(payload: dict = Body(default={})):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=M3M_MIS_Report.xlsx"},
    )
...
def export_pdf_route(payload: dict = Body(default={})):
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=M3M_MIS_Full_Report.pdf"},
    )
...
def export_pdf_onepager_route(payload: dict = Body(default={})):
    ...
    return StreamingResponse(
        io.BytesIO(pbytes), media_type="application/pdf",
        headers={"Content-Disposition": "attachment; filename=M3M_MIS_Executive_OnePager.pdf"},
    )
