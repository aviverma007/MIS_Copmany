...
Usage:
    ...
    python test_backend.py /path/to/M3M_Sample_Template.xlsx
"""
import sys
import time
...
def main(sample_file: str):
    ...
    print(f"load_and_process: {len(df):,} rows, {len(meta['missing_fields'])} missing fields "
          f"({time.time() - t0:.2f}s)")
    assert meta["missing_fields"] == [], f"Unexpected missing fields: {meta['missing_fields']}"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python test_backend.py /path/to/M3M_Sample_Template.xlsx")
        sys.exit(1)
    main(sys.argv[1])
