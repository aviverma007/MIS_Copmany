"""
Smartworld MIS - Filtering, KPI, report, insight, attention & data-quality logic.
...
def build_reports(df: pd.DataFrame) -> dict:
    reports = {
        ...
        "area": _group_report(df, "area"),
        "sub_area": _group_report(df, "sub_area"),
    ...
    # Top/Bottom 5 "categories" are based on whichever of Service Category /
    # Area actually carries data - Area, for a Smartworld-style export where
    # Service Category is blank; Service Category otherwise. This keeps the
    # Overview tab's Top 5/Bottom 5 Categories cards meaningful regardless of
    # which field the uploaded workbook actually populates.
    cat = reports["service_category"] if any(r["total"] for r in reports["service_category"]) else reports["area"]
...
def management_attention(df: pd.DataFrame) -> dict:
    ...
    # Whichever category field actually carries data for this workbook - see
    # the same fallback in build_reports() / management_insights().
    cat_field = "service_category" if ("service_category" in df.columns and df["service_category"].notna().any()) else "area"

    missing_cols = [c for c in ["case_owner", "project_name", cat_field] if c in df.columns]
    if missing_cols:
...
def data_quality(df: pd.DataFrame) -> dict:
    ...
    checks["blank_service_category"] = int(df[cat_field].isna().sum()) if cat_field in df.columns else 0
...
def management_insights(df: pd.DataFrame) -> list:
    ...
    # Prefer Service Category if it actually carries data; fall back to Area
    # (Smartworld's export leaves Service Category blank and uses Area instead).
    cat_field = "service_category" if ("service_category" in df.columns and df["service_category"].notna().any()) else "area"
    if cat_field in df.columns and df[cat_field].notna().any():
        top_cat = df.groupby(cat_field)["case_number"].nunique().sort_values(ascending=False)
        if len(top_cat):
            insights.append(f"'{top_cat.index[0]}' is the highest-volume category with {int(top_cat.iloc[0])} cases.")
